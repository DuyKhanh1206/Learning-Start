...
namespace プログラムSQLine
{
    public partial class frmAddPhoneNumber : Form
    {
        ...
        private string _strSqrlite;
        private string _sqliteConn;
        private ClsSQLite _sqlite;
        public frmAddPhoneNumber()
            {
                InitializeComponent();
            _sqliteConn = @"Data Source=S:\装置開発部\■共有\91_個人\湯浅\900_勉強結果\02_電話帳\DB\DDD.db;Version=3";

            _sqlite = new ClsSQLite(_sqliteConn);
        ...
        private void bntRegister_Click(object sender, EventArgs e)
            {
                ...
                if( dt.Rows.Count == 0)
                    {
                        _strSqrlite = string.Format("insert into phones values({0},{1},{2})",  NumPhoneID.Value, txtUserID.Text , txtPhoneNumber.Text);
                        _sqlite.ExecuteNoneQuery(_strSqrlite);
